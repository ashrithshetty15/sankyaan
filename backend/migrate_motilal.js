...
async function migrateMotilal() {
  try {
    console.log('🔄 Adding Motilal Oswal portfolio to unified table...\n');
    ...
    `);
    console.log(`Current Motilal Oswal records: ${beforeCount.rows[0].count}`);

    // Read and execute SQL
    const sql = fs.readFileSync('add_motilal_to_unified.sql', 'utf8');
    ...
    const afterCount = await db.query(`
      ...
      WHERE fund_house = 'Motilal Oswal'
    `);
    console.log(`New Motilal Oswal records: ${afterCount.rows[0].count}`);
    ...
    const funds = await db.query(`
      ...
      LIMIT 10
    `);
    console.log('\nSample Motilal Oswal funds in unified table:');
