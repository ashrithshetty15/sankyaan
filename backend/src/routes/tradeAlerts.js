...
export async function triggerScan(req, res) {
  try {
    ...
    // Build event map for IV Crush scanner
    // IV Crush entries are only valid in the 10:15–11:30 AM IST window
    const ist = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
    const istMins = ist.getHours() * 60 + ist.getMinutes();
    const inIVCrushWindow = istMins >= 615 && istMins <= 690; // 10:15=615, 11:30=690

    const upcomingEvents = inIVCrushWindow ? await getAllUpcomingEvents(5) : [];
    ...
    const alerts = await scanStrategies(underlyings, expiry, eventMap);
