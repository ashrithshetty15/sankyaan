...
export default function Home({ viewMode, setViewMode }) {
  ...
  const [fundChanges, setFundChanges] = useState(null);
  const [changesTab, setChangesTab] = useState('added');
  ...
  const performSearch = async (ticker) => {
    ...
    try {
      ...
      try {
        const chResp = await axios.get(API_URL + '/fund-changes/' + encodeURIComponent(ticker));
        setFundChanges(chResp.data);
      } catch (_) { setFundChanges(null); }
    } catch (err) {
      ...
      {fundData && (
        <div id="fund-report-container" className="results-section">
          ...
          <div className="portfolio-view">
            ...
            {/* Holdings + Changes Row */}
            <div className="holdings-changes-row">

            {/* Portfolio Changes Section */}
            {fundChanges && fundChanges.available && (
              <div className="fund-changes-section">
                <div className="fund-changes-header">
                  <div>
                    <h3 className="holdings-title">Portfolio Changes</h3>
                    <p className="holdings-subtitle">
                      {new Date(fundChanges.oldDate).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
                      {' → '}
                      {new Date(fundChanges.newDate).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
                      {' · '}
                      <span className="changes-badge added">{fundChanges.added.length} added</span>
                      {' '}
                      <span className="changes-badge removed">{fundChanges.removed.length} removed</span>
                      {' '}
                      <span className="changes-badge increased">{fundChanges.increased.length} increased</span>
                      {' '}
                      <span className="changes-badge decreased">{fundChanges.decreased.length} decreased</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="changes-tabs">
                  {[
                    { key: 'added',     label: 'Added',     count: fundChanges.added.length,     color: '#22c55e' },
                    { key: 'removed',   label: 'Removed',   count: fundChanges.removed.length,   color: '#ef4444' },
                    { key: 'increased', label: 'Increased', count: fundChanges.increased.length, color: '#3b82f6' },
                    { key: 'decreased', label: 'Decreased', count: fundChanges.decreased.length, color: '#f59e0b' },
                  ].map(tab => (
                    <button
                      key={tab.key}
                      className={'changes-tab' + (changesTab === tab.key ? ' active' : '')}
                      style={changesTab === tab.key ? { borderBottomColor: tab.color, color: tab.color } : {}}
                      onClick={() => setChangesTab(tab.key)}
                    >
                      {tab.label}
                      {tab.count > 0 && <span className="changes-tab-count">{tab.count}</span>}
                    </button>
                  ))}
                </div>
                ...
                <div className="changes-list">
                  {changesTab === 'added' && (
                    fundChanges.added.length === 0
                      ? <p className="changes-empty">No new positions added</p>
                      : fundChanges.added.map((h, i) => (
                        <div key={i} className="changes-row added-row">
                          <span className="changes-pill added-pill">NEW</span>
                          <span className="changes-name">{h.name}</span>
                          <span className="changes-industry">{h.industry}</span>
                          <span className="changes-pct added-pct">+{h.pctNav.toFixed(2)}%</span>
                        </div>
                      ))
                  )}
                  {changesTab === 'removed' && (
                    fundChanges.removed.length === 0
                      ? <p className="changes-empty">No positions exited</p>
                      : fundChanges.removed.map((h, i) => (
                        <div key={i} className="changes-row removed-row">
                          <span className="changes-pill removed-pill">EXIT</span>
                          <span className="changes-name">{h.name}</span>
                          <span className="changes-industry">{h.industry}</span>
                          <span className="changes-pct removed-pct">{h.pctNav.toFixed(2)}%</span>
                        </div>
                      ))
                  )}
                  {changesTab === 'increased' && (
                    fundChanges.increased.length === 0
                      ? <p className="changes-empty">No positions increased</p>
                      : fundChanges.increased.map((h, i) => (
                        <div key={i} className="changes-row increased-row">
                          <span className="changes-pill increased-pill">▲</span>
                          <span className="changes-name">{h.name}</span>
                          <span className="changes-industry">{h.industry}</span>
                          <span className="changes-pct increased-pct">{h.oldPct.toFixed(2)}% → {h.newPct.toFixed(2)}%</span>
                          <span className="changes-delta increased-delta">+{h.delta.toFixed(2)}%</span>
                        </div>
                      ))
                  )}
                  {changesTab === 'decreased' && (
                    fundChanges.decreased.length === 0
                      ? <p className="changes-empty">No positions decreased</p>
                      : fundChanges.decreased.map((h, i) => (
                        <div key={i} className="changes-row decreased-row">
                          <span className="changes-pill decreased-pill">▼</span>
                          <span className="changes-name">{h.name}</span>
                          <span className="changes-industry">{h.industry}</span>
                          <span className="changes-pct decreased-pct">{h.oldPct.toFixed(2)}% → {h.newPct.toFixed(2)}%</span>
                          <span className="changes-delta decreased-delta">{h.delta.toFixed(2)}%</span>
                        </div>
                      ))
                  )}
                </div>
